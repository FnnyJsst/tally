...
export default function NewMovementScreen() {
  ...
  const { productId: preselectedProductId, variantId: preselectedVariantId } = useLocalSearchParams<{ productId?: string; variantId?: string }>()
  ...
  const [selectedVariant, setSelectedVariant] = useState<string>(preselectedVariantId ?? '')
  ...
  useEffect(() => {
    if (preselectedVariantId && products.length > 0 && !selectedProduct) {
      const parent = products.find(p => p.variants?.some(v => v.id === preselectedVariantId))
      if (parent) setSelectedProduct(parent.id)
    }
  }, [preselectedVariantId, products])

  useEffect(() => {
    if (product?.variants && product.variants.length > 0 && !preselectedVariantId) {
  ...
  const handleSubmit = async () => {
    if (!selectedProduct) { Alert.alert('Champ manquant', 'Sélectionne un produit.'); return }
    if (!selectedVariant) { Alert.alert('Champ manquant', 'Sélectionne une variante.'); return }
    if (!selectedChannel) { Alert.alert('Champ manquant', 'Sélectionne un canal.'); return }
    if (quantity <= 0) { Alert.alert('Quantité invalide', 'La quantité doit être supérieure à 0.'); return }

    setIsLoading(true)
    const { error } = await addMovement({
      variantId: selectedVariant, channelId: selectedChannel,
      quantity, type, note: note.trim() || undefined,
    })
    setIsLoading(false)
        ...
        <ScrollView contentContainerStyle={styles.form} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.typeGrid}>
            {MOVEMENT_TYPES.map((t) => (
              ...
              >
                <Text style={[styles.typeSign, type === t.value && styles.typeSignActive]}>{t.sign}</Text>
            ...
            {products.length === 0 ? (
              ...
              products.map((p, i) => (
                <View key={p.id}>
                  ...
                  <TouchableOpacity style={styles.pickerRow} onPress={() => setSelectedProduct(p.id)}>
                    <Text style={styles.pickerLabel}>{p.name}</Text>
                    {selectedProduct === p.id && <Text style={styles.checkmark}>✓</Text>}
                  </TouchableOpacity>
                </View>
              ))
            )}
            ...
            <>
              ...
              <View style={styles.card}>
                {product.variants.map((v, i) => (
                  <View key={v.id}>
                    {i > 0 && <View style={styles.separator} />}
                    <TouchableOpacity style={styles.pickerRow} onPress={() => setSelectedVariant(v.id)}>
                      <Text style={styles.pickerLabel}>{v.name}</Text>
                      {selectedVariant === v.id && <Text style={styles.checkmark}>✓</Text>}
                    </TouchableOpacity>
                  </View>
                ))}
              </View>
          ...
          <View style={styles.card}>
            {channels.length === 0 ? (
              ...
              channels.map((c, i) => (
                <View key={c.id}>
                  {i > 0 && <View style={styles.separator} />}
                  <TouchableOpacity style={styles.pickerRow} onPress={() => setSelectedChannel(c.id)}>
                    <Text style={styles.pickerLabel}>{c.name}</Text>
                    {selectedChannel === c.id && <Text style={styles.checkmark}>✓</Text>}
                  </TouchableOpacity>
                </View>
              ))
            )}
...
function makeStyles(colors: ColorScheme, isDark: boolean) {
  return StyleSheet.create({
    container: { flex: 1, backgroundColor: 'transparent', paddingTop: 60 },
    topbar: {
    ...
    typeChip: {
      flex: 1, minWidth: '30%', paddingVertical: Spacing.md,
    ...
    typeSign: { fontSize: FontSize.lg, color: colors.text2 },
    typeSignActive: { color: isDark ? '#C4ADEC' : '#FFF' },
    typeLabel: { fontSize: FontSize.xs, fontWeight: '500', color: colors.text2 },
    typeLabelActive: { color: isDark ? '#C4ADEC' : '#FFF' },
    card: {
      backgroundColor: isDark ? 'rgba(255,255,255,0.07)' : colors.surface,
    ...
    emptyPicker: { fontSize: FontSize.sm, color: colors.text3, padding: Spacing.md, textAlign: 'center' },
    qtyRow: {
      flexDirection: 'row', alignItems: 'center', justifyContent: 'center',
      gap: Spacing.xl, paddingVertical: Spacing.md,
    },
    qtyBtn: {
    ...
    qtyValue: {
      fontSize: 28, fontWeight: '300', color: colors.text, letterSpacing: -1,
      minWidth: 60, textAlign: 'center', fontVariant: ['tabular-nums'],
    },
    noteInput: {
